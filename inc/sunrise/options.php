<?php
	...
	$options = array(
		...
		array(
			'name' => __( 'Position', $this->textdomain ),
			'desc' => __( '<b>Relative</b> will add the ribbon into the flow of the document, which will often give the best results. However, this does require adding "hooks" to the template. First add the following immediately after the opening &lt;body&gt; tag: &lt;? do_action("okf_panel"); ?&gt; Then add this where you want the ribbon to appear (should be near the top of the page): &lt;? do_action("okf_ribbon"); ?&gt;', $this->textdomain ),
			'options' => array(
				'absolute' => __( 'Absolute', $this->textdomain ),
				'fixed' => __( 'Fixed', $this->textdomain ),
				'relative' => __( 'Relative (requires template editing, see below)', $this->textdomain )
			),
			'std' => 'relative',
			'id' => 'position',
			'type' => 'radio'
		...
		*/
		...
		/*
		array(
			'name' => __( 'Samples', $this->textdomain ),
			'type' => 'opentab'
		),
		array(
